        ...
        public void Clip(List<Vector2> clipPolygon, List<Vector2> subjectPolygon, in List<Vector2> clipResult)
        {
            ...
            foreach (LineSegment clipEdge in IterateSegmentCCW(clipPolygon))
            {
                ...
                foreach (LineSegment lineSegment in IterateSegmentCCW(_input))
                {
                    ...
                    if (intersection.CheckState(Geometry2D.SegmentIntersection.ClipState.StartInside))
                    {
                        _output.Add(intersection.Clip.Start);

                        // 이게 교차점을 구하다보면, 시작지점과 끝점이 같아질 수가 있다.
                        // 가령, 시작지점은 선위에 걸쳐져있는데, 선이 서로 수직인 경우가 있을 수 있다.
                        if (intersection.CheckState(Geometry2D.SegmentIntersection.ClipState.EndInside) == false)
                        {
                            if (intersection.Clip.Start != intersection.Intersection)
                                _output.Add(intersection.Intersection);
                        }
                    }
                    else if (intersection.CheckState(Geometry2D.SegmentIntersection.ClipState.EndInside))
                    {
                        _output.Add(intersection.Intersection);
                    }
                }
